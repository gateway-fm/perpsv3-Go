...
func TestService_RetrieveTrades_OnChain(t *testing.T) {
	...
	sBlock := uint64(13739029)
	lBlock := uint64(13739029)

	res, err := s.RetrieveTrades(sBlock, &lBlock)
	...
	want := &models.Trade{
		...
		BlockNumber:      sBlock,
		//TODO: add normal timestamp!!
		BlockTimestamp:  0,
		TransactionHash: "0x16704162005c11d71c745f7392a71a5ede8eb5f042e7fa917f210748773c57bf",
	}

	require.NoError(t, err)
	require.Equal(t, want.MarketID, res[0].MarketID)
	require.Equal(t, want.AccountID, res[0].AccountID)
	require.Equal(t, want.FillPrice, res[0].FillPrice)
	require.Equal(t, want.PnL, res[0].PnL)
	require.Equal(t, want.AccruedFunding, res[0].AccruedFunding)
	require.Equal(t, want.SizeDelta, res[0].SizeDelta)
	require.Equal(t, want.NewSize, res[0].NewSize)
	require.Equal(t, want.TotalFees, res[0].TotalFees)
	require.Equal(t, want.ReferralFees.Bytes(), res[0].ReferralFees.Bytes())
	require.Equal(t, want.CollectedFees.Bytes(), res[0].CollectedFees.Bytes())
	require.Equal(t, want.SettlementReward.Bytes(), res[0].SettlementReward.Bytes())
	require.Equal(t, want.TrackingCode, res[0].TrackingCode)
	require.Equal(t, want.Settler, res[0].Settler)
	require.Equal(t, want.BlockNumber, res[0].BlockNumber)
	require.Equal(t, want.BlockTimestamp, res[0].BlockTimestamp)
	require.Equal(t, want.TransactionHash, res[0].TransactionHash)
}
